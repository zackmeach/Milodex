...
Item {
    ...
    // ------------------------------------------------------------------
    // Layout: 4-column grid of Surface panels.  Wraps gracefully when
    // the window narrows (Grid.flow: GridFlow.LeftToRight is implicit).
    // ------------------------------------------------------------------
    ...
    Grid {
        ...
        Surface {
            ...
            Column {
                ...
                // Stale / error indicator at the bottom of the panel
                Text {
                    width: parent.width
                    visible: root._brokerError || root._staleData
                    text: root._brokerError
                            ? ("Error: " + OperationalState.brokerErrorMessage)
                            : ("Stale -- last refresh "
    ...
    Item {
        ...
        Surface {
            ...
            height: Math.min(parent.height - Theme.space[6] * 2, 240)
            ...
            Column {
                ...
                Row {
                    ...
                    Button {
                        ...
                        text:    "Yes, reset"
                        onClicked: {
                            OperationalState.reset_kill_switch("CONFIRM")
