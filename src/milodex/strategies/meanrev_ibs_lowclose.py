...
Implements the ``meanrev`` family, template ``daily.ibs_lowclose``, per
``docs/strategy-families.md``:

- Long-only daily swing on broad index ETFs
- Entry: above the ``ma_filter_length`` SMA and IBS below
  ``ibs_entry_threshold``
- Exit: close > prior day's high (when enabled), ``max_hold_days``
  reached, or close-based stop triggered
- Ranking: ``ibs_ascending`` (lowest IBS first) when more candidates than
  open capacity

IBS is ``(close - low) / (high - low)``, the position of the close within
the day's range. Low IBS = close near the low = oversold. The signal is
unreliable on single names (see docs) and is intended for broad /
sector ETFs only; universe choice enforces that here.

Cross-sectional family: ``context.bars_by_symbol`` carries the per-symbol
``BarSet``; the ``bars`` argument to ``evaluate`` is unused.
"""
...
class MeanrevIbsLowcloseStrategy(Strategy):
    """Internal-Bar-Strength daily mean-reversion strategy."""

    family = "meanrev"
    template = "daily.ibs_lowclose"
    parameter_specs = (
        StrategyParameterSpec("ibs_entry_threshold", expected_types=(int, float)),
        StrategyParameterSpec("prior_high_exit_enabled", expected_types=(bool,)),
    ...
    def evaluate(self, bars: BarSet, context: StrategyContext) -> StrategyDecision:
        ...
        # Scope open positions to the strategy's declared universe (ADR 0021
        # guardrail at the strategy layer; broker account is shared in paper).
...
def _validated_parameters(context: StrategyContext) -> dict[str, Any]:
    ...
    return {
        ...
        "prior_high_exit_enabled": bool(required("prior_high_exit_enabled")),
...
def _market_regime_is_bullish(
    ...
    Missing regime data fails *open* (allows entries) rather than silently
    blocking every cycle — the risk layer remains authoritative for hard
    stops. Matches the ``meanrev.daily.pullback_rsi2`` behaviour.
    """
...
def _entry_candidates(
    *,
    ...
    """Return (symbol, latest_close, ibs) for each eligible entry candidate."""
    ...
    for symbol in universe:
        ...
        if latest_high <= latest_low:
            rejected_alternatives.append(
                {
                    "symbol": normalized,
                    "reason": "zero-range bar (high == low); IBS undefined",
                }
            )
            continue
        ibs = (latest_close - latest_low) / (latest_high - latest_low)
...
def _exit_intents(
    ...
    for symbol, quantity in open_positions.items():
        ...
        # Order: stop_loss > max_hold > prior-high-break (most specific first).
        ...
        if (
            exit_rule is None
            and parameters["prior_high_exit_enabled"]
            and len(highs) >= 2
            and latest_close > float(highs.iloc[-2])
        ):
...
def _exit_narrative(rule: str, symbol: str, parameters: dict[str, Any]) -> str:
    if rule == "meanrev.ibs_exit":
        return f"close broke prior day's high → sell {symbol}"
...
def _exit_threshold(rule: str, parameters: dict[str, Any]) -> dict[str, float | int | str | None]:
    if rule == "meanrev.ibs_exit":
        return {"prior_high_exit_enabled": parameters["prior_high_exit_enabled"]}
    if rule == "meanrev.stop_loss":
