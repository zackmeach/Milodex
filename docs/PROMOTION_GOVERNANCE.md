...
- **[machine]** the required backtest report metrics (`metrics_snapshot`: Sharpe, max-drawdown, trade count, round-trip count) — sourced from the run's **OOS-aggregate** when the run is walk-forward, or from whole-period metrics otherwise. Per [ADR 0021](adr/0021-walk-forward-metrics-are-oos-aggregate.md), the gate reads OOS-aggregate values so it cannot be satisfied by in-sample lucky stretches. The gate is **stage-aware** per [ADR 0028](adr/0028-stage-aware-promotion-gate.md): paper-readiness uses Sharpe > 0.0, max drawdown < 25%, ≥ 15 round-trips; live-readiness keeps the strict Sharpe > 0.5, max drawdown < 15%, ≥ 30 round-trips. The asymmetry reflects the asymmetric cost — a missed strategy at paper-readiness scraps an edge for $0 risk-savings; a wrongly-promoted strategy at live-readiness risks real capital. Benchmark comparisons per R-BKT-005 / R-ANA- remain artifact-side
