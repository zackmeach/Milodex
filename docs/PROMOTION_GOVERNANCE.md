...
- **[machine]** gate outcome (`gate_check_outcome.promotion_type`: `statistical` or `lifecycle_exempt`; `failures` list) — confirms the strategy cleared the minimum evidence thresholds **for its strategy class** (research-target per R-PRM-004, or the operational gate for the lifecycle-proof strategy). Authoritative numeric threshold definitions (two-tier: paper-readiness and capital-readiness) live in `src/milodex/promotion/policy.py` (ADR 0052).
