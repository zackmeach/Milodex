...
- **C-1.** *(operative — §4.1.i chosen)* A second research-target strategy moves through the full lifecycle — define → frozen manifest → backtest → walk-forward → trust report — with the same evidence shape as meanrev's Phase 1 trip. Walk-forward report uses the per-metric `(OOS)` labeling discipline P-1 added. The strategy either earns the gate (Sharpe > 0.5, max drawdown < 15%, configured trade-count floor) or is refused honestly per the same C-2 mechanism Phase 2 locked. Family choice: momentum per §4.4; strategy ID `momentum.daily.tsmom.curated_largecap.v1`.
