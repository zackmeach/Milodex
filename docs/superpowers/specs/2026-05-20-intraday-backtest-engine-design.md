...
Milodex's `BacktestEngine` is daily-only by construction. It hardcodes `timeframe=Timeframe.DAY_1` in [`BacktestEngine.prefetch_bars`](../../src/milodex/backtesting/engine.py) when fetching universe bars, and iterates `for day in trading_days` in [`BacktestEngine._simulate_daily`](../../src/milodex/backtesting/engine.py), calling `strategy.evaluate()` once per trading day with a daily bar slice.
...
**Code reference.** The explanatory comment in `src/milodex/backtesting/engine.py` inside `BacktestEngine._simulate_intraday` (search for "advance → evaluate → drain" in the event-loop body) documents this reasoning inline for implementers reading the code directly.
...
- Intraday helpers: `src/milodex/backtesting/intraday_simulation.py` (extracted from `engine.py` 2026-05-24 — `_build_intraday_event_timeline`, `_opens_at_timestamp`, `_mark_to_market_at_day_end`, `_build_visible_bars`, `_latest_close_at_ts`, `_advance_cursors`)
