...
3. **For each event timestamp T in the day, in chronological order, the order of operations is strictly:**
   1. **Drain pending orders if T is a fill event:** for each pending order whose symbol has a bar with `bar_timestamp == T` (i.e., its open is observable now), fill at that bar's open with slippage and commission applied. Orders whose symbol has no bar at T remain pending — they wait for the next future fill event for that symbol.
   2. **Advance cursors if T is a decision event:** for each symbol with a bar whose `decision_time == T`, advance `cursors[symbol]` by 1 so that bar is now in visible history. After this step, `bars_df.iloc[:cursor[symbol]]` includes every bar whose `decision_time` is ≤ T.
   3. **Evaluate the strategy ONLY if step 2 advanced any cursor** (i.e., T is a decision event for at least one symbol). The strategy sees the post-advancement multi-symbol visible history. New intents append to `pending`. If T is a pure fill event (drain happened but no cursors advanced), no evaluation runs — there is no new bar information for the strategy to react to.

   This ordering guarantees the conservative invariants: (a) orders created at T cannot fill at T because the fill phase for T already completed before the strategy could create them; (b) no lookahead — bars whose `decision_time > T` are excluded from the strategy's visible history; (c) no underdraw — bars whose `decision_time == T` ARE visible, because they're complete at T by definition.
...
4. **Per-symbol cursors** — `cursors: dict[str, int]`. Cursor invariant: `cursor[symbol]` is the **exclusive end index** of the symbol's bar history currently visible to the strategy. Visible history is always `bars_df.iloc[:cursor[symbol]]`. **Initialization:** at simulation start, `cursors[symbol] = 0` for every universe symbol (no history visible yet). **Advancement timing — load-bearing:** at each event timestamp T in the timeline, the order of operations is strictly:
   1. **Drain pending orders if T is a fill event for any symbol:** for each pending order whose symbol has a bar with `bar_timestamp == T`, fill at that bar's open with slippage and commission applied. Symbols without a fill event at T leave their pending orders untouched.
   2. **Advance cursors if T is a decision event for any symbol:** for each symbol with a bar whose `decision_time == T`, advance `cursors[symbol]` by 1. After this step, `bars_df.iloc[:cursor[symbol]]` includes every bar whose `decision_time` is ≤ T for that symbol.
   3. **Evaluate the strategy ONLY if step 2 advanced any cursor** (T is a decision event for at least one symbol). The strategy sees the post-advancement visible history. New intents append to `pending`. Pure fill events (no decision events at T) do not trigger evaluation — there's no new bar information for the strategy to act on.

   This ordering guarantees no lookahead (bars whose `decision_time > T` are excluded from the strategy's view) AND no underdraw (bars whose `decision_time == T` ARE visible, because they're complete by definition at T). It also preserves the conservative invariant "orders created at T cannot fill at T" — new intents from step 3 enter `pending` *after* step 1's drain phase, so they're only eligible at the next future fill event. Cursors persist across timestamps within a day and across day boundaries — they advance monotonically through the chronological event timeline.
