...
| Iconography as primary signal | Color and language carry the signal. Icons are sparingly used and never load-bearing. (See `SegmentedToggle.qml`: "type and colour only, no iconography". The `GateTable.qml` example that also illustrated this was retired 2026-07-17 with the dead showcase code.) |
