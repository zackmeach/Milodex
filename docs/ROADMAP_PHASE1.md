...
**Live status:** see §2 (success criteria) and the checkboxes in §4–§6 for the authoritative completion state. This header is intentionally pointer-only so it cannot drift from the checklists below. As of 2026-04-26: Phase 1.0–1.3 complete end-to-end, Phase 1.4 slices 1–3 complete (frozen manifest, state machine + evidence, live-stage refusal). **SC-1, SC-2, SC-4, SC-5, SC-6 closed.** Remaining gating work is **SC-3's meanrev half** — the live paper-session shakeout (§8 item 7) requires US market hours and is the *only* code-and-bookkeeping item between us and Phase 1 completion. The §7 cross-cutting items continue to track non-gating reporting / coverage / docs work (walk-forward labeling and stage-source consistency closed 2026-04-26).
