...
`--configs` and `--strategy-id` are mutually exclusive. `--configs` discovery selects only registered, runnable strategy configs: non-strategy YAMLs such as universe manifests and risk defaults are skipped, and schema-valid templates whose `(family, template)` is not registered are skipped with a reason. Explicit `--strategy-id` values are not discovery-filtered; invalid or unregistered strategy IDs still surface as screen errors. `--report-out` with no value writes to `docs/reviews/screen_<today>.md` (plus a JSON sibling at the same stem); pass an explicit path to override. `--fail-fast` aborts on the first actual run error instead of recording it as an `error` row — default is to continue so one failing strategy does not mask results for the other candidates.
...
**Output.** Human-readable ranking table sorted by (gate_allowed desc, oos_sharpe desc). Columns: `strategy_id | family | trades | oos_sharpe | oos_max_dd | fragile | surv_corr | gate`. `fragile=yes` means dropping the best-returning window flips aggregate return negative (ADR 0021 single-window-dependency flag). `surv_corr` discloses whether the declared universe manifest is survivorship-corrected. `gate=pass (statistical)` / `pass (lifecycle_exempt)` / `block` / `error`. When discovery skips any matched YAMLs, human and report output include a compact skipped-config summary.

**JSON contract.** `research.screen` payload: `start_date`, `end_date`, `row_count`, `rows[]` (each row includes the metrics and gate failure list), `report_path`, `matched_config_count`, `selected_strategy_ids`, `skipped_configs`, and `correlation_matrix`. Stable across invocations so downstream tooling can ingest screening snapshots.
