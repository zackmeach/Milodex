...
## Resolution Log (2026-06-20)

Operator answered the §5 decisions: **1 approve · 2 confirm time-stop-only · 3 yes · 4 build now · 5 "is this my call?"** (#5 was not a decision — it's the execution order that follows from 1/3/4; orchestrator-owned).

| # | Decision | Resolution |
|---|---|---|
| **N1** | boundary clamp | DONE `ccd6810` — `rng.integers` high → `OR+EW−_BAR_MINUTES+1` (max target 325). |
| **N2** | stranded-exit | DONE `75fe447` (+`120754c` wording) — **engine session-end force-flatten** (`kernel.liquidate_open_positions`), mirrors drain SELL accounting, skips pending self-exits, intraday-only. **risk-invariant review = SAFE (7/7).** Scope chosen: engine force-flatten (the strategy-level ≥-exit is provably insufficient at `exit_min=5`, where the time-stop bar *is* the last RTH bar). |
| **N3** | baseline_ref | DONE `ccd6810` — fan-out rewrites `baseline_ref` per symbol; all 17 configs verified correct. |
| **N4** | gap-scan on_grid | DONE `ccd6810` — `_max_intra_session_gap(on_grid, …)`. |
| **2** | hold-duration | CONFIRMED time-stop-only / count-matched; brief updated (clause dropped). |
| **3 (Fix C)** | snapshot batching | **REVERSED — no-op.** Measured: `record_final_snapshot` is once-per-sweep; cProfile of the Fix-A+B branch shows **zero** sqlite/commit/fsync in the hot path. Remaining ~67% is pandas tz-conversion (`_local_timestamps`), CPU-bound, not batchable. Not built. The gate report's per-bar-snapshot-fsync hypothesis was wrong. **Consequence:** working-lane run is CPU-bound, parallelizes cleanly with no disk contention. |
| **4** | F/G/H build-now | PROCEEDING. F-PR1 (experiment registry, migration 015) DONE `3819ab5`, risk-invariant SAFE; operator signed off the schema. |
