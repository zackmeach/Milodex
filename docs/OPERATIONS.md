...
> **Daily execution — resolved via queue-at-open (D-1 → [ADR 0057](adr/0057-daily-execution-queue-at-open.md), Accepted 2026-06-22).** Evaluating the post-close bar produces a *decision*, not a *trade*: the risk evaluator's market-hours check (`_check_market_open`, [`src/milodex/risk/evaluator.py:437`](../src/milodex/risk/evaluator.py), reason `market_closed` at `:445`) still vetoes every closed-market submit. Rather than weaken that sacred check, the daily lifecycle is **split** — at the post-close cycle the runner persists the locked-in intent to the append-only `queued_intents` table (migration 016) **instead of** submitting; at the next market open it drains its own active intents, **rebuilds sizing against fresh state**, and re-routes each through `submit_paper`, where the complete 17-check risk battery re-runs (only `_check_market_open` flips BLOCK→ALLOW). A queued intent is a proposal, never a pre-approved order. Scope is the **conservative fail-closed drop**: at drain time an uncertain halt/LULD read or an unreconciled prior-session partial **DROPs** the intent with an audit row, and a dropped *exit* raises a durable operator alert (surfaced, never silently stranded). M1's bar is **one named fill plus the daily branch-proof**, not a high daily fill-rate; full halt/LULD detection + partial-fill reconciliation are an explicit fast-follow.
