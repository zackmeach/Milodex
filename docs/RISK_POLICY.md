...
- **Operator CLI trades (`milodex trade`) are bound only by the account-scoped caps — even when `--strategy-config` is passed.** The per-strategy check keys on the runner-bound `expected_max_positions` envelope (a runner-startup TOCTOU snapshot, deliberately `None` for operator manual trades — see `TradeIntent` in `src/milodex/execution/models.py`), so it skips operator submissions rather than re-reading the per-cycle YAML value the TOCTOU design distrusts. Before 2026-07-15 the account-scoped check *incidentally* clamped such trades to the strategy's YAML `max_positions`; that clamp was the ADR 0024 spec≠code defect removed in PR #364, not a designed guarantee. The trade is still recorded under the strategy's name, so it lands in that strategy's ledger and counts against the strategy's own cap on its next runner intent — the exemption is bounded and fails closed for the strategy, never open.
...
- **The increasing/reducing split governs the exposure *projections*, not just exemptions (A-6, 2026-06-16).** The **single-position** and **total-exposure** caps now project a naked short or a sell-beyond-held by adding its **uncovered (excess) leg** to projected exposure, instead of netting the full order notional off the held value (which let a short read as benign exposure-reducing notional and slip past both caps). A fully-covered exit still nets down exactly as before. All three caps (order-value, single-position, total-exposure) share `risk/exposure.py` (`is_exposure_increasing` / `exposure_increasing_notional`) so they agree on exposure direction. The projection assumes `estimated_order_value == estimated_unit_price × quantity` (how `ExecutionRequest` is built). Phase-1 strategies are long-only, so this is defense-in-depth against a buggy/short/fat-fingered SELL, not a currently-exercised path.
...
3. **In-flight order accounting is same-process and account-scoped — cross-process race now closed for all non-backtest submits (ADR 0056, amended 2026-06-15).** `_check_total_exposure` / `_check_concurrent_positions` count open BUYs from `context.recent_orders` (ADR 0024), which closes the same-process burst-before-fill overshoot. The **cross-process** evaluate→submit race (two runners' intents both passing against a stale position snapshot and both filling) is closed by a per-account submit-serialization advisory lock that wraps the read-snapshot→evaluate-caps→submit span ([ADR 0056](adr/0056-cross-process-submit-serialization-per-account-advisory-lock.md), implemented in `execution/service.py`; bounded blocking acquire, fail-closed with reason `submit_serialization_unavailable`). As of 2026-06-15 the lock engages for **paper too** (not just `micro_live`/`live`): the concurrent-intraday-runners work lets many paper runners share one account, including several on one symbol, so paper carries the same stale-snapshot race. Only backtests stay lock-free (simulated broker, single process). The lock makes Milodex's own submit sections mutually exclusive per account; it does not make caps atomic against asynchronous broker fills — paper now carries the same residual the live path always did, economically nil at 5Min × ~11 strategies, sub-$1k intents, $101k account. (The `recent_orders` truncation gap in #4 and per-strategy P&L in #6 remain independently required.)
4. **`recent_orders` is broker-truncated (`get_orders(limit=100)`) with no durable backstop for the cap checks.** Unlike `_check_duplicate_order` (which since PR5 queries the durable event store directly, scoped per-strategy, and no longer reads `recent_orders` at all), the exposure/slot checks consume only the truncated list, so >100 in-window orders could drop a pending BUY (undercount). Bounded in Phase 1; add an event-store-backed open-order query at the live gate.
5. **The per-strategy concurrent cap reads the strategy ledger (concurrent-intraday PR3, 2026-06-15).** On the live/paper path `_check_strategy_concurrent_positions` now derives the owned set from `strategy_positions(proposing_strategy, event_store)` — the strategy-scoped submitted-fill ledger ([ADR 0055](adr/0055-event-store-per-strategy-position-ledger.md) amendment) — rather than re-attributing broker-net positions. This both (a) fixes the sibling-netting fail-open (a symbol netted flat at the account is still seen in the strategy's own ledger) and (b) counts a strategy's prior **submitted** orders, since the ledger folds `submitted`/`accepted`/`filled` rows — so an in-flight (recorded-but-unfilled) order now occupies a per-strategy slot, where the old broker-net mechanism counted only filled positions. Residual: an order submitted to the broker but not yet *recorded* (e.g. a recording failure window) is still uncounted; and the account-scoped in-flight gap (#4, `recent_orders` truncation) is separate. **Partial-fill quantity** is now reconciled on the **synchronous-fill path** — `_record_execution` records `result.order.filled_quantity` (and `filled_avg_price × filled_quantity`) whenever the broker reports a fill at submit time (backtest, instant-fill brokers, synchronously-reported partial fills), so the ledger reflects the actual fill, not the requested intent. **Still open (async path):** an Alpaca paper market order returns PENDING at submit, so its fill arrives later and is only ever reflected by the corrective-row reconciliation (`operations/reconciliation.py` `sync_local_only_orders`), which copies the *local requested* quantity, and by the attribution fold, which reads the original row — neither reconciles to broker `filled_qty`. So an async **partial** fill (e.g. a market order that partial-fills then the remainder cancels) still mis-states the lot until that deeper reconciliation+fold change lands. Fail-safe and nil in the current paper regime (liquid symbols + market orders fully fill). The backtest path keeps the broker-net + `attribute_position` reconstruction (no sibling netting in a single-strategy replay; `strategy_positions` is paper-source-only).
...
- `R-EXE-014` through `R-EXE-017` encode the scope split, the post-trip requirements, the reducing-vs-increasing asymmetry, and the hard-stop vs warning classification. **DC-1 (2026-06-10):** R-EXE-015(f) and R-EXE-016 have been amended in the SRS to match this document and the code. The kill switch is an absolute halt; reducing-order permissiveness applies to the order-value cap and the reconciliation gate only, not to the kill-switch check.
