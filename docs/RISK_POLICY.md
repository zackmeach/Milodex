...
3. **In-flight order accounting is same-process and account-scoped — cross-process race now closed for `micro_live`/`live` (ADR 0056).** `_check_total_exposure` / `_check_concurrent_positions` count open BUYs from `context.recent_orders` (ADR 0024), which closes the same-process burst-before-fill overshoot. The **cross-process** evaluate→submit race (two runners' intents both passing against a stale position snapshot and both filling) is now closed at `micro_live`/`live` by a per-account submit-serialization advisory lock that wraps the read-snapshot→evaluate-caps→submit span ([ADR 0056](adr/0056-cross-process-submit-serialization-per-account-advisory-lock.md), implemented in `execution/service.py`; bounded blocking acquire, fail-closed with reason `submit_serialization_unavailable`). Paper stays lock-free by design — no real capital, the bounded overshoot is accepted — and the lock path is dormant in Phase 1 until those stages open. (The `recent_orders` truncation gap in #4 and per-strategy P&L in #6 remain independently required.)
