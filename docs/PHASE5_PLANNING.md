...
1. **Design-system token set.** First Phase 5 GUI PR. Color palette + semantic roles, typography (font family, scale, weight, line height), spacing scale, motion duration tiers + easing, elevation/shadow tiers. Versioned artifact applied consistently across every QML surface.
