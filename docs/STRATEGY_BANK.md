...
> **Config-file `stage:` vs promotion records — don't be surprised by 43.** A `grep 'stage: paper' configs/` returns **43** files, not 11. The extra 32 are non-SPY intraday ETF replicas — two full 16-symbol families, `benchmark.unconditional_intraday_long.<etf>` and `meanrev.rsi2.intraday.<etf>` — that carry `stage: paper` in YAML but have **no promotion record and no frozen manifest**. They are *not* part of the paper roster: promotion records are this document's binding source for stage (see below), and the risk layer vetoes any unfrozen strategy with `no_frozen_manifest` regardless of a config's YAML `stage:` field. The freeze decision for these 32 is roadmap item **D-2**.
...
> **Reproducible again (guard removed 2026-06-15):** that soak test launched all 5 SPY intraday canaries (+ the regime daily strategy) at once — all six resolve `universe[0]=SPY`. The ADR-0055 launch-time eval-symbol guard that once refused a second same-symbol runner **was removed 2026-06-15** (see *"Concurrency — same-symbol co-run is now allowed"* above); the three invariants it proxied are now closed in the risk/execution layers. Same-symbol, same-account co-run is **allowed**, so the canaries can again be run concurrently.
