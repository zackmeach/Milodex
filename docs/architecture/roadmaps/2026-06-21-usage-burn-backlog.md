...
|----|-------|-----|-------|------|--------|------|----------|
| A1 | `CONFIRMED-OPEN` | now | Consolidate 4-way RSI / 2-way ATR / EMA onto `_indicators.py` (parity tests FIRST) | A | small | low→med | high |
| A2 | `UNVERIFIED` | park | Extract shared session-intraday evaluation flow (8 strategies) | A | large | med | high |
| A3 | `UNVERIFIED` | park | Consolidate 22× `_validated_parameters` boilerplate onto shared coercer | A | large | med | high |
| A4 | `UNVERIFIED` | now | Unit-test the cross-sectional feature kit (`_decider_features.py`) | A | small | low | high |
| A5 | `UNVERIFIED` | park | Add strategies-test conftest with shared barset/context factories | A | decent | low | high |
| B1 | `UNVERIFIED` | park | Split the 82-method `EventStore` god-class behind a stable facade | B | large | med | high |
| B2 | `UNVERIFIED` | stretch | Decompose 2766-line `bench.py` into per-action-family modules | B | large | med | high |
| B3 | `UNVERIFIED` | now | Collapse 6 repeated propose/submit Qt slot pairs (RM-008) | B | small | low | high |
| C1 | `UNVERIFIED` | now | Convert ~28 source-substring pins in `test_qml_load_smoke.py` to behavioral (pilot first) | C | large | low | high |
| C2 | `UNVERIFIED` | now | De-brittle 34 source-pins in the two reachability test files (pilot first) | C | decent | low | high |
| C3 | `UNVERIFIED` | now | Un-quarantine `test_design_system_showcase` + resolve 4 xfails | C | small | low | med |
| D1 | `UNVERIFIED` | now | Add **deduped** logging to ~11 silent `sqlite3.Error` swallows (GUI read layer) | D | small | low | high |
| D2 | `CONFIRMED-OPEN` | now | Doc-debt sweep (3 independent sub-items: CLAUDE.md refs · STRATEGY_BANK · coverage) | D | small | low | high |
| D3 | `UNVERIFIED` | park | De-dup reconciliation serializers/money-formatter vs `cli/_shared` (after D4) | D | small | low | high |
| D4 | `UNVERIFIED` | now | Unit-test `cli/_shared` JSON serializers & formatters (do before D3) | D | small | low | high |
| D5 | `UNVERIFIED` | park | Finish GUI-hardening P9/P13 (2 independent sub-items: BenchSurface · EvidenceModal) | D | small | low | high |
| E1 | `RESCOPED` | — | Intraday data-readiness report — superseded by merged `intraday_readiness.py` | E | — | low | — |
| E2 | `RESCOPED` | — | Baseline/null framework — superseded by merged `evidence_assembler.py` | E | — | low | — |
| F1 | `UNVERIFIED` | now | Clean-room install + first-launch audit | F | decent | low | high |
| F2 | `UNVERIFIED` | now | End-to-end paper lifecycle rehearsal (setup→…→explanation) | F | large | med | high |
| F3 | `UNVERIFIED` | now | Failure/recovery drills → actionable operator explanations | F | decent | med | high |
