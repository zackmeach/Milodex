# ADR 0046 - Kanban stage hues extend production tokens
...
5. **The Kanban QML port waits for token implementation.** No Kanban QML file should inline prototype hex colors or bespoke grain values. Stage tokens land in `Theme.qml`, all theme files, the design-system doc, and the showcase before the board depends on them.
...
- The first implementation PR should add stage tokens before adding Kanban visuals that need them.
