...
**(f) Per-strategy position attribution** is CS-1 path (a) — the option [ADR 0024](0024-account-scoped-position-caps-are-authoritative.md) explicitly deferred with the note: *"if/when the operator opens that goal, option (a) becomes the right next move and this ADR's semantics extend rather than reverse."* Phase 4 opens that goal. The mechanics-firming intent and the concrete operational pain the operator named (can't tell what strategies have made or lost) both point here: attribution at the risk layer is a prerequisite for any future surface — CLI, GUI, or otherwise — to report per-strategy P&L accurately. ADR 0029 will articulate the new semantics in detail; this ADR only authorizes the work.
