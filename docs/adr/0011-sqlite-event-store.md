...
- **Single file, no server, stdlib.** Matches Milodex's "no services, no daemons" posture. Backup is `cp data/milodex.db data/milodex.db.bak`; sharing is zipping the file. The operator can open the same file in any SQL GUI (DB Browser for SQLite, DBeaver) or drive it from `sqlite3` on the command line without running the app.
