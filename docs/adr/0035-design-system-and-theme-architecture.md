...
## Addendum 2026-07-17: DesignSystemShowcase retired as dead code

The `DesignSystemShowcase` surface — and its showcase-only components `StrategyRow`, `StatusPill`, and `GateTable` — were retired as dead code. The showcase had become unreachable in the shipped app: no tab or navigation path sets `activeSurface` to `"design-system"`, and `StrategyRow`/`StatusPill` were instantiated only inside the showcase (`GateTable` nowhere). Their now-orphaned `Theme` tokens (`typography.display.md`/`smItalic`, the `ease` easing curves, and the `column.pill`/`metric`/`chips`/`tradeCount`/`kanban*`/`benchConfigKey`/`benchStage`/`benchEvidence` widths) were removed with them; the `Button` and `Surface` foundational components and all three curated themes (Editorial Dark/Light, Bronze) are unaffected. The ADR-0035 QML-load integration smoke, previously pinned to the showcase, now pins a live surface (`BenchSurface.qml`) under the same bundled-fonts + registered-singletons subprocess bootstrap. This addendum records the retirement; the design principles above stand unchanged.
