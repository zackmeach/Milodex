...
1. **Tempo-aware staleness** (config-derived, evaluator-side). At the open a daily strategy's latest 1D bar is the *prior* session's close (≫ the global 300s `max_data_staleness_seconds`), so every daily resubmit would veto `stale_market_data`. `_check_data_staleness` therefore derives its budget **inside the evaluator** from the **authoritative resolved config** on the context (`context.strategy_config.bar_size`): `1D` → one-session budget; `None` / non-`1D` (operator manual intent, intraday) → 300s. The selector is a property of the resolved config, **never a field on the intent** (I-2) — an intraday intent provably cannot inherit the daily budget. (This required adding a `bar_size` field to `StrategyExecutionConfig`, which had no `tempo`/`bar_size` — a BLOCKER enabler.)
