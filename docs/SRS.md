...
- **Lifecycle-proof strategy** — the SPY/SHY 200-DMA regime strategy used to validate the *platform* end-to-end. Its promotion gates are operational (lifecycle milestones reached, controls verified), not statistical. It is intentionally exempt from the research-target statistical gates in R-PRM-004, because a regime strategy generates only 1–3 signals per year.
- **Research-target strategy (edge-family target)** — strategies intended to discover actual edge. Mean reversion is the first in Phase 1. Research-target strategies are subject to the stage-aware statistical thresholds in R-PRM-004, with trade-count evidence sourced from each strategy's `backtest.min_trades_required` setting.
...
| R-EXE-004 | The `RiskEvaluator` **shall** enforce these checks: kill switch active, paper-mode enforcement, strategy stage eligibility, market hours, data staleness, daily loss cap (kill-switch threshold and strategy-level), order-value fat-finger, single-position size, total portfolio exposure, **sector exposure cap *(planned — not yet enforced)*, correlated-idea exposure cap *(planned — not yet enforced)*,** concurrent positions cap, **config fingerprint match,** **broker-vs-local state reconciliation,** duplicate-order detection. The live enforced set is the `RiskEvaluator._CHECKS` tuple (14 today); the two caps marked *planned* are design targets gated to live-capital readiness — see `docs/RISK_POLICY.md` "Known limitations". See `docs/RISK_POLICY.md` for numeric thresholds and "Hard Stops vs Warnings" for the classification of each check. | Each enforced check has a dedicated unit test covering pass and fail cases; `test_checks_registry_is_account_complete` guards the set against silently listing an unimplemented cap. |
...
| R-EXE-013 | The system **shall** prevent two runners from starting against the **same `strategy_id`** by acquiring a per-strategy advisory lock (`milodex.runtime.strategy.<strategy_id>`) under `data/locks/` as resolved by `milodex.config.get_locks_dir()` for the duration of the runner. A second `milodex strategy run <strategy_id>` invocation against the same `strategy_id` **shall** be refused with a clear error identifying the running process (PID, hostname, start time). Different `strategy_id`s **shall** be permitted to run concurrently per ADR 0026 (per-process supervisor model); account-state safety for concurrent strategies relies on the broker via the risk evaluator's per-call position query (ADR 0024), not on inter-process locks. **History:** in Phase 1 this requirement enforced "at most one strategy runs at a time" via a single global `milodex.runtime` lock; that restriction was lifted in Phase 3 (see ADR 0026 and the Phase 2+ appendix entry below). | Tests: starting the same `strategy_id` twice refuses with `advisory_lock_held` and identifies the prior runner; starting two different `strategy_id`s under the same `locks_dir` both succeed (`tests/milodex/cli/test_main.py::test_strategy_run_refuses_second_invocation_of_same_strategy`, `::test_strategy_run_allows_concurrent_different_strategies`). |
...
| R-EXE-015 | A kill-switch trip **shall** produce a reviewable incident record, not merely a halt flag. The record **shall**: (a) capture the exact triggering condition, (b) snapshot local state, broker state, and affected strategy state, (c) mark scope (strategy vs account) in durable state, (d) surface an operator-facing incident summary, (e) be linked by ID to any subsequent re-enable governance event (per `docs/PROMOTION_GOVERNANCE.md` reversal pattern), (f) permit read-only inspection during the halt (CLI status, GUI read-only surfaces). **DC-1 (2026-06-10):** the original clause (f) included "exposure-reducing actions during the halt" — this is rescinded. The kill switch is an absolute halt: no orders are submitted while it is active, reducing or otherwise. See `docs/RISK_POLICY.md` "Reducing vs Increasing Exposure". No auto-resume path **shall** exist. | Integration test: a synthetic trip writes a complete incident record with all listed fields; read-only CLI commands succeed during the halt; reset requires an explicit operator command that writes a linked governance event. |
| R-EXE-016 | The `RiskEvaluator` **shall** distinguish **exposure-increasing** orders from **exposure-reducing** orders and apply policy asymmetrically per `docs/RISK_POLICY.md` "Reducing vs Increasing Exposure". Exposure-increasing orders run the full check set. Exposure-reducing orders (Phase 1: sells that close or shrink existing long positions) run a more permissive set. **DC-1 (2026-06-10):** the original requirement included "shall remain permitted while an account-level kill switch is active" — this is rescinded. The kill switch is an absolute halt (code and RISK_POLICY agree; the prior YAML `reducing_allowed_during_kill_switch: true` was fiction, corrected in HR-6). Reducing orders are permissive for the order-value cap and the reconciliation gate; they are NOT permissive for the kill-switch check. | Tests: an ordinary closing sell is blocked while the kill switch is active; an increasing buy is also blocked; reducing-sell exemption applies to order_value cap and reconciliation gate only. |
...
| R-BKT-004 | The backtester **shall** refuse to report "pass" status on any run with fewer than the strategy's configured `backtest.min_trades_required` value (default 30). | Test: with the default floor, a 29-trade backtest returns `statistically_insufficient: true`; with a configured floor of 20, 20 trades satisfy the trade-count check and 19 trades do not. |
...
| R-PRM-004 | Advancing a **research-target** strategy to `paper` **shall** require paper-readiness evidence: Sharpe > 0.0, max drawdown < 25%, and trade count at or above the strategy's configured `backtest.min_trades_required` value (default 30). Advancing a research-target strategy beyond `paper` **shall** require the stricter capital-readiness gate: Sharpe > 0.5, max drawdown < 15%, and the same configured trade-count floor. The **lifecycle-proof** strategy (per Key Terms) is exempt from these statistical thresholds because a regime strategy cannot produce 30 trades in a realistic backtest window; its `paper` gate requires instead (a) a successful deterministic backtest run, (b) explanation records (R-XC-008) generated for every simulated signal, and (c) the risk layer having rejected at least one synthetic fault-injection trade. Both gate types **shall** be recorded in the `promotion_log`. Implementation source of truth: `src/milodex/promotion/policy.py` (ADR 0052). | Test: a research-target strategy that meets paper-readiness but not capital-readiness can promote to `paper` but not to `micro_live`; a configured trade floor of 20 allows 20 trades and blocks 19; the lifecycle-proof strategy is allowed to advance on its operational gate without meeting a statistical trade-count threshold. |
...
- ~~Lift the Phase 1 single-strategy restriction (R-EXE-013).~~ **Implemented in Phase 3 per [ADR 0026](adr/0026-concurrent-multi-strategy-uses-per-process-supervisor.md).** Multiple strategies run simultaneously against the same Alpaca account via the per-process supervisor model — each strategy in its own foreground `milodex strategy run <strategy_id>` process, with the runner's advisory lock scoped per `strategy_id` so disjoint strategies do not block each other while the same `strategy_id` still refuses a second invocation. R-EXE-013 above carries the current testable contract.
- Portfolio-level risk checks (exposure, concurrent positions) continue to apply globally per [ADR 0024](adr/0024-account-scoped-position-caps-are-authoritative.md); per-strategy risk stays scoped to the strategy. Account-state safety for concurrent strategies is the broker's responsibility, surfaced via the risk evaluator's per-call position query — not file locks.
- **Capital allocation model (proposed):** a single shared pool, with each strategy's allocation weighted by its validated reliability (Sharpe, drawdown stability, trade count). High-confidence strategies receive a larger slice; new strategies start small and earn share. This is directional intent for Phase 4+, not a committed design.
- Controlled-stop and kill-switch semantics: controlled stop is per-process (Ctrl-C in the relevant terminal); kill switch remains account-wide per [ADR 0005](adr/0005-kill-switch-manual-reset.md) — a kill switch tripped in one runner refuses the next `evaluate_intent` in the other runner with `kill_switch_active`.
