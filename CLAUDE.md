...
- **`tests/milodex/gui/test_app.py::test_main_qml_loads_without_errors_via_subprocess` is a known env failure** — PySide6 ships no bundled fonts in this venv (`QFontDatabase: Cannot find font directory …/PySide6/lib/fonts`) and the test asserts the QML subprocess emits empty stderr. Pre-existing/environmental (fails identically on a clean `master`); a full-suite run reporting exactly `1 failed` is almost certainly this. Don't debug it as a regression.
- **Strategy config validation is `strategies/loader.py:load_strategy_config`, NOT `cli/config_validation.py`** — the `_STRATEGY_REQUIRED_KEYS` dict there is stale/unused (says `name`/`universe`; real configs use `id`/`universe_ref`). Required `strategy.` keys: id, family, template, variant, version, description, enabled, parameters, tempo, risk, stage, backtest, disable_conditions_additional. `id` must equal `{family}.{template}.{variant}.v{version}`; **new family strings are accepted (no allowlist)** — a new strategy = `loader.py build_default_registry()` + one `configs/*.yaml` + the `strategies/*.py` class.
...
- **Per-strategy DB trade rows ≠ broker net when strategies share one account (ADR 0055).** Alpaca nets per account+symbol; a sibling's sell can flatten broker net while your strategy's submitted `trades` still show an open lot (2026-06-03: rsi2 +13, vwap_trend −13 → account flat). The runner now derives `context.positions` and `entry_state` from the **strategy-scoped** event-store ledger (`strategy_positions` / `strategy_open_lots` in `risk/attribution.py`) — so runner positions are **not** Alpaca net by design during concurrent same-symbol operation. Account-level risk caps still use live broker positions. Reconciliation surfaces per-strategy vs broker divergence as informational WARN only (not R-OPS-004 incidents). Wash-trade (`40310000`) risk remains; same-symbol co-run is **launch-time code-enforced** on evaluation symbol (`context.universe[0]`) via `milodex strategy run` — stop the existing runner before starting another strategy on the same symbol. Cross-process submit serialization and per-symbol advisory locks are deferred (ADR 0026 addendum).
