...
- **Per-strategy DB trade rows ≠ broker net when strategies share one account (ADR 0055).** Alpaca nets per account+symbol; a sibling's sell can flatten broker net while your strategy's submitted `trades` still show an open lot (2026-06-03: rsi2 +13, vwap_trend −13 → account flat). The runner now derives `context.positions` and `entry_state` from the **strategy-scoped** event-store ledger (`strategy_positions` / `strategy_open_lots` in `risk/attribution.py`) — so runner positions are **not** Alpaca net by design during concurrent same-symbol operation. Account-level risk caps still use live broker positions. Reconciliation surfaces per-strategy vs broker divergence as informational WARN only (not R-OPS-004 incidents). Same-symbol co-run is now **allowed** (the launch-time eval-symbol guard was removed 2026-06-15, ADR 0026 addendum superseded): the three invariants it proxied are closed in the risk/execution layers — PR1 paper submit serialization (ADR 0056 amended), PR2 opposite-side resting-order veto (`_check_opposite_side_order`), PR3 per-strategy cap reads the strategy ledger (ADR 0055 amended). Cross-process submit serialization is now in place for all non-backtest submits — `paper`/`micro_live`/`live` (per-account advisory lock, ADR 0056); per-symbol advisory locks remain deferred. Two fail-safe same-symbol residuals remain (neither unsafe): (1) partial-fill ledger reconciliation (`RISK_POLICY.md` #5) — fails closed for the cap, nil in the paper regime; (2) `_check_duplicate_order` / `count_recent_submitted_orders` are **account-wide, not per-strategy** (no `strategy_name` predicate) despite the spec/config comment saying per-strategy — so two different strategies' legitimate same-side entries on one symbol within the 60s window false-veto the second (skipped trade, not unsafe). See ADR 0026 addendum 2026-06-15.
