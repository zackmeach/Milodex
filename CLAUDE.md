...
- **Promotion pipeline is mandatory.** Stages: backtest → paper → micro_live → live. No skipping stages. Thresholds are stage-aware (ADR 0028): paper-readiness uses Sharpe > 0.0, max drawdown < 25%, ≥ 15 round-trips; live-readiness (paper→micro_live, micro_live→live) keeps the strict bar at Sharpe > 0.5, max drawdown < 15%, ≥ 30 round-trips. Round-trip count (closed positions) is the statistical-power input.
