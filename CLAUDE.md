...
- **5Min cache holds 17 ETF symbols** at `market_cache/v3/5Min/<SYM>.parquet` (DIA, GLD, IWM, QQQ, SPY, TLT, XLB, XLC, XLE, XLF, XLI, XLK, XLP, XLRE, XLU, XLV, XLY — the intraday-evidence universe). Any new intraday strategy on a symbol outside this set needs cache warmup first. Legacy daily cache at `market_cache/1Day/` (unversioned) coexists with the v3 layout.
...
- **GUI subprocess QML tests both PASS now; the residual non-pass class is 4 xfails, not a skip.** Fonts are bundled (`src/milodex/gui/assets/fonts/`, `gui/fonts.py:load_fonts`), so BOTH subprocess QML smoke tests pass — `test_app.py::test_main_qml_loads_without_errors_via_subprocess` and `test_design_system_showcase_loads_without_errors_via_subprocess` (the latter **un-quarantined 2026-06-21**, was `@pytest.mark.skip`; same font root cause; verified green across 10 consecutive full-suite xdist runs). A clean full `tests/milodex/gui` run reads `847 passed, 4 xfailed` — **no skips from this class.** The 4 xfails are the inline-instantiation tests in `test_qml_components.py` (Button-danger / StatusPill paper+killed / StrategyRow): process-global QML *type-cache* pollution (the Milodex module compiles before QtQuick.Layouts resolves), still unfixed, `@pytest.mark.xfail(strict=False)` so they show as xfailed/xpassed and NEVER as `failed`. Treat an actual `N failed` as a real regression, not this. See `docs/KNOWN_FLAKY_TESTS.md`.
