...
- **Promotion CLI: `milodex promotion promote ...`**, not `milodex promote` (refused legacy shortcut). `--lifecycle-exempt` is **scoped** (ADR 0058): it bypasses the statistical gate ONLY for policy-listed lifecycle-proof strategy ids (`ACTIVE_PROMOTION_POLICY.lifecycle_gate.applies_to` = the SPY/SHY regime strategy); a lifecycle-exempt promotion of any other id is refused (fail closed) and pointed at `--operator-override`. The **general** bypass is `--operator-override` — paper-stage only (capital stages belong to the autonomy boundary), requires a non-blank `--recommendation` as the recorded reason, and lands as `promotion_type='operator_override'`. The two flags are mutually exclusive. Scoping + override enforcement live in `promotion/orchestrator.py` (`_check_bypass_admissibility`); `check_gate` (`promotion/state_machine.py`) is UNCHANGED — its `if lifecycle_exempt:` short-circuit still returns `allowed=True`, and `policy.py`'s `LifecycleGateDefinition` stays define-only (`enforced=False`). Enforcement of the three R-PRM-004 criteria (a)(b)(c) is **deferred to roadmap M4** (ADR 0058); until then a lifecycle-exempt promotion durably records those criteria as unenforced (`gate_check_outcome.lifecycle_criteria`, `deferred='M4'`). Historical pre-ADR-0058 `lifecycle_exempt` rows keep their old unscoped semantics (append-only ledger) — read them via the date boundary.
...
- **A green full suite reads 0 skipped (plus a few xfailed).** The old "1 skipped is expected" baseline is gone: the showcase subprocess test (`test_app.py::test_design_system_showcase_loads_without_errors_via_subprocess`) was un-quarantined in `f019a3f` (root cause was the missing-fonts flake, fixed by bundled fonts in `src/milodex/gui/assets/fonts/` via `gui/fonts.py:load_fonts`; history in `docs/KNOWN_FLAKY_TESTS.md`). Any `skipped` or `failed` in a clean full run is a real regression to investigate, not expected noise.
