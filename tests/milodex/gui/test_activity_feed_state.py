...
def test_error_after_success_preserves_last_known(qapp, tmp_path) -> None:
    """After a successful refresh, a failure leaves the last-known rows intact."""
    _ = qapp
    db = tmp_path / "feed.db"
    _create_fixture_db(db)

    now = datetime.now(tz=UTC)
    _seed_explanation(db, recorded_at=now.isoformat(), risk_allowed=1)
...
def test_stop_drains_in_flight_worker(qapp, tmp_path) -> None:
    ...
    from milodex.gui.activity_feed_state import _ActivityFeedRefreshRunnable
    ...
    original_run = _ActivityFeedRefreshRunnable.run
    ...
    _ActivityFeedRefreshRunnable.run = slow_run

    try:
        state._kick_refresh()  # noqa: SLF001
        assert worker_ran.wait(timeout=3.0), "Worker did not start within 3s"

        # Worker is genuinely in-flight (blocking on release).  Schedule unblock
        # *after* stop() starts so that stop() must actually wait.
        ...
        _ActivityFeedRefreshRunnable.run = original_run
