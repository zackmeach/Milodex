...
@_skip_no_qt
def test_desk_section_composition_net() -> None:
    """All seven DeskSurface top-level section Columns must be present and
    carry their stable objectNames.

    Walks the full QQuickItem tree after load, collects all objectName strings,
    and asserts every name in DESK_SECTION_OBJECT_NAMES is present.  If any
    is absent the process exits non-zero and names the missing ones.

    This is the composition regression net: rename or drop a section Column
    while leaving the headless load-smoke gate green → this test fails.
    """
    script = _HARNESS_F.format(
        import_root=repr(str(_QML_IMPORT_ROOT)),
        desk=repr(str(_DESK_SURFACE)),
        expected=repr(list(DESK_SECTION_OBJECT_NAMES)),
    )
    _run(script, "DeskSurface section composition net")


# Single source of truth for the seven pinned objectNames.
DESK_SECTION_OBJECT_NAMES = (
    "deskSectionRiskMode",
    "deskSectionPerformance",
    "deskSectionActiveOps",
    "deskSectionRiskThroughput",
    "deskSectionAttention",
    "deskSectionMarketTape",
    "deskSectionOrderTape",
)
...
# Collect every objectName present in the tree.
found = set()
for it in _walk(root):
    name = it.property("objectName")
    if name:
        found.add(str(name))
...
print("DESK_SECTION_COMPOSITION_OK (" + str(len(expected)) + " sections found)")
sys.exit(0)
"""
